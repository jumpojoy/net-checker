...
def load_json_files(folder):
    ...
    files = {f.replace('.json', ''): f for f in os.listdir(folder) if f.endswith('.json')}
    ...
    for scenario, filename in files.items():
        with open(os.path.join(folder, filename), 'r') as f:
            ...
            if scenario.lower().startswith('tcp'):
                ...
                scenario_data[scenario] = {'protocol': 'tcp', 'sent_bps': sent_bps, 'recv_bps': recv_bps}
                ...
                scenario_data[scenario] = {'protocol': 'udp', 'bps': udp_bps, 'lost_percent': lost_percent}
...
def plot_tcp_diagram(folder_data, output_file='tcp_scenario_comparison.svg'):
    """Generate a bar diagram for TCP sent bits per second across folders."""
    # Collect TCP scenarios
    tcp_scenarios = sorted(set(s for folder in folder_data.values() for s in folder 
    ...
    # Colors for different folders (extendable for more folders)
    colors = ['red', 'green', 'blue', 'coral', 'yellow']
    
    for i, folder_name in enumerate(folders):
        sent_bps = [folder_data.get(folder_name, {}).get(s, {'sent_bps': 0})['sent_bps'] / 1_000_000_000 
                    for s in tcp_scenarios]
        
        # Plot sent bars, positioned similarly to UDP
        offset = (i - num_folders/2 + 0.5) * width
        ax.bar(x + offset, sent_bps, width, 
               label=f'{folder_name}: Sent (Gbps)', color=colors[i])
...
def plot_udp_diagram(folder_data, output_file='udp_scenario_comparison.svg'):
    """Generate a bar diagram for UDP bits per second with vertically printed lost packet percentages."""
    # Collect UDP scenarios
    udp_scenarios = sorted(set(s for folder in folder_data.values() for s in folder 
    ...
    for i, folder_name in enumerate(folders):
        bps = [folder_data.get(folder_name, {}).get(s, {'bps': 0})['bps'] / 1_000_000_000 
               for s in udp_scenarios]
        lost_percent = [folder_data.get(folder_name, {}).get(s, {'lost_percent': 0})['lost_percent'] 
                        for s in udp_scenarios]
        
        # Plot bars with clear folder name in legend
        bars = ax.bar(x + (i - num_folders/2 + 0.5) * width, bps, width, 
                      label=f'{folder_name}: Throughput (Gbps)', color=colors[i])
...
def main():
    ...
    targets = {}
    scenario_folders = [x for x in os.listdir(net_data) if os.path.isdir(os.path.join(net_data, x))]
    ...
    for source_target, scenarios in targets.items():
        ...
        out_file = f"{source}_{target}.svg"
        ...
        if any(s for folder in folder_data.values() for s in folder if folder[s].get('protocol') == 'tcp'):
            plot_tcp_diagram(folder_data, os.path.join(out_dir, f"tcp_{out_file}"))
        
        # Generate UDP bar diagram if UDP scenarios exist
        if any(s for folder in folder_data.values() for s in folder if folder[s].get('protocol') == 'udp'):
            plot_udp_diagram(folder_data, os.path.join(out_dir, f"udp_{out_file}"))
